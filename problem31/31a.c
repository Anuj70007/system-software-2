...
#define PROJECT_ID 'A'  // Project ID for generating unique key
...
int main() {
    ...
    // Step 3: Initialize the binary semaphore to 1 (unlocked state)
    if (initialize_semaphore(semid, 1) == -1) {
        exit(EXIT_FAILURE);
    }

    printf("Binary semaphore created and initialized to 1 (unlocked state).\n");
